...
typedef enum {
    ...
    COL_DIAMETER,
    COL_BIPARTITE,
    COL_VERTEX_CONNECTIVITY,
    COL_EDGE_CONNECTIVITY,
    COL_AUTOMORPHISM_GROUP_ORDER,
    COL_TRIANGLE_COUNT,
    COL_FOUR_CYCLE_COUNT,
    COL_FIVE_CYCLE_COUNT,
    COL_SIX_CYCLE_COUNT,
    COL_ADJACENCY_SPECTRUM,
    COL_LAPLACIAN_SPECTRUM,
    COL_SPANNING_TREE_COUNT,
    COL_SPECTRUM,
    COL_GAMMA,
    COL_G_MAX,
    COL_EXPECTED_GENUS,
    ...
    COL_R_1,
    COL_R_2,
    COL_TOTAL_ROTATION_SYSTEMS,
    COL_HAS_NONREAL_ROOTS,
    COL_COUNT
} ColumnId;
...
static const ColumnSpec COLUMN_SPECS[COL_COUNT] = {
    {COL_INDEX, "index"},
    {COL_GRAPH6, "graph6"},
    {COL_N, "n"},
    {COL_M, "m"},
    ...
    {COL_BIPARTITE, "bipartite"},
    {COL_VERTEX_CONNECTIVITY, "vertex_connectivity"},
    {COL_EDGE_CONNECTIVITY, "edge_connectivity"},
    {COL_AUTOMORPHISM_GROUP_ORDER, "automorphism_group_order"},
    {COL_TRIANGLE_COUNT, "triangle_count"},
    {COL_FOUR_CYCLE_COUNT, "four_cycle_count"},
    {COL_FIVE_CYCLE_COUNT, "five_cycle_count"},
    {COL_SIX_CYCLE_COUNT, "six_cycle_count"},
    {COL_ADJACENCY_SPECTRUM, "adjacency_spectrum"},
    {COL_LAPLACIAN_SPECTRUM, "laplacian_spectrum"},
    {COL_SPANNING_TREE_COUNT, "spanning_tree_count"},
    {COL_SPECTRUM, "spectrum"},
    {COL_GAMMA, "gamma"},
    {COL_G_MAX, "g_max"},
    {COL_EXPECTED_GENUS, "expected_genus"},
    {COL_VARIANCE, "variance"},
    {COL_R, "R"},
    {COL_R_1, "R_1"},
    {COL_R_2, "R_2"},
    {COL_TOTAL_ROTATION_SYSTEMS, "total_rotation_systems"},
    {COL_HAS_NONREAL_ROOTS, "has_nonreal_roots"},
};
...
typedef struct {
    ...
    bool diameter_ready;
    int diameter;
    bool bipartite_ready;
    bool bipartite;
    bool vertex_connectivity_ready;
    int vertex_connectivity;
    bool edge_connectivity_ready;
    int edge_connectivity;
    bool automorphism_ready;
    char automorphism_order[128];
    bool cycles_ready[7];
    uint64_t cycle_counts[7];
    bool adjacency_spectrum_ready;
    char adjacency_spectrum[GP_FIELD_CAP];
    bool laplacian_spectrum_ready;
    char laplacian_spectrum[GP_FIELD_CAP];
    bool spanning_trees_ready;
    char spanning_trees[128];
    bool genus_ready;
    GenusStats genus_stats;
    bool has_nonreal_roots_ready;
    bool has_nonreal_roots;
} AnalysisContext;
...
static bool genus_polynomial_has_nonreal_roots(const GenusStats *stats) {
    fmpz_poly_t poly;
    fmpz_poly_t squarefree;
    fmpz_poly_init(poly);
    fmpz_poly_init(squarefree);

    for (int genus = 0; genus < GP_MAX_GENUS; ++genus) {
        if (stats->counts[genus]) {
            fmpz_poly_set_coeff_ui(poly, genus, (ulong)stats->counts[genus]);
        }
    }

    fmpz_poly_squarefree_part(squarefree, poly);
    slong degree = fmpz_poly_degree(squarefree);
    bool has_nonreal = false;
    if (degree > 0) {
        slong real_roots = fmpz_poly_num_real_roots_vca(squarefree);
        has_nonreal = real_roots < degree;
    }

    fmpz_poly_clear(squarefree);
    fmpz_poly_clear(poly);
    return has_nonreal;
}

static bool ensure_genus_stats(AnalysisContext *ctx) {
    if (ctx->genus_ready) return true;
    if (!compute_genus_stats(ctx->g, &ctx->genus_stats, ctx->err, ctx->err_len)) {
        return false;
    }
    ctx->genus_ready = true;
    return true;
}

static bool append_csv_field(char *row, size_t row_cap, size_t *pos,
                             const char *value) {
    int written = snprintf(row + *pos, row_cap - *pos, "%s%s", *pos ? "," : "",
                           value);
    if (written < 0 || (size_t)written >= row_cap - *pos) return false;
    *pos += (size_t)written;
    return true;
}

static bool append_csv_column(char *row, size_t row_cap, size_t *pos,
                              AnalysisContext *ctx, ColumnId column) {
    char tmp[256];
    const Graph *g = ctx->g;

    switch (column) {
    case COL_INDEX:
        snprintf(tmp, sizeof(tmp), "%zu", g->index);
        return append_csv_field(row, row_cap, pos, tmp);
    case COL_GRAPH6:
        return append_csv_field(row, row_cap, pos, g->graph6);
    case COL_N:
        snprintf(tmp, sizeof(tmp), "%d", g->n);
        return append_csv_field(row, row_cap, pos, tmp);
    case COL_M:
    ...
    case COL_GIRTH:
        if (!ctx->girth_ready) {
            ctx->girth = compute_girth(g);
            ctx->girth_ready = true;
        }
        snprintf(tmp, sizeof(tmp), "%d", ctx->girth);
        return append_csv_field(row, row_cap, pos, tmp);
    case COL_DIAMETER:
        if (!ctx->diameter_ready) {
            ctx->diameter = compute_diameter(g);
            ctx->diameter_ready = true;
        }
        snprintf(tmp, sizeof(tmp), "%d", ctx->diameter);
        return append_csv_field(row, row_cap, pos, tmp);
    case COL_BIPARTITE:
        if (!ctx->bipartite_ready) {
            ctx->bipartite = is_bipartite(g);
            ctx->bipartite_ready = true;
        }
        return append_csv_field(row, row_cap, pos, ctx->bipartite ? "True" : "False");
    case COL_VERTEX_CONNECTIVITY:
        if (!ctx->vertex_connectivity_ready) {
            ctx->vertex_connectivity = vertex_connectivity(g);
            ctx->vertex_connectivity_ready = true;
        }
        snprintf(tmp, sizeof(tmp), "%d", ctx->vertex_connectivity);
        return append_csv_field(row, row_cap, pos, tmp);
    case COL_EDGE_CONNECTIVITY:
        if (!ctx->edge_connectivity_ready) {
            ctx->edge_connectivity = edge_connectivity(g);
            ctx->edge_connectivity_ready = true;
        }
        snprintf(tmp, sizeof(tmp), "%d", ctx->edge_connectivity);
        return append_csv_field(row, row_cap, pos, tmp);
    case COL_AUTOMORPHISM_GROUP_ORDER:
        if (!ctx->automorphism_ready) {
            compute_automorphism_order(g, ctx->automorphism_order,
                                       sizeof(ctx->automorphism_order));
            ctx->automorphism_ready = true;
        }
        return append_csv_field(row, row_cap, pos, ctx->automorphism_order);
    case COL_TRIANGLE_COUNT:
    case COL_FOUR_CYCLE_COUNT:
    case COL_FIVE_CYCLE_COUNT:
    case COL_SIX_CYCLE_COUNT: {
        int length = column == COL_TRIANGLE_COUNT ? 3
                   : column == COL_FOUR_CYCLE_COUNT ? 4
                   : column == COL_FIVE_CYCLE_COUNT ? 5
                                                   : 6;
        if (!ctx->cycles_ready[length]) {
            ctx->cycle_counts[length] = count_cycles_length(g, length);
            ctx->cycles_ready[length] = true;
        }
        snprintf(tmp, sizeof(tmp), "%" PRIu64, ctx->cycle_counts[length]);
        return append_csv_field(row, row_cap, pos, tmp);
    }
    case COL_ADJACENCY_SPECTRUM:
        if (!ctx->adjacency_spectrum_ready) {
            compute_spectrum_string(g, false, ctx->adjacency_spectrum,
                                    sizeof(ctx->adjacency_spectrum));
            ctx->adjacency_spectrum_ready = true;
        }
        return append_csv_field(row, row_cap, pos, ctx->adjacency_spectrum);
    case COL_LAPLACIAN_SPECTRUM:
        if (!ctx->laplacian_spectrum_ready) {
            compute_spectrum_string(g, true, ctx->laplacian_spectrum,
                                    sizeof(ctx->laplacian_spectrum));
            ctx->laplacian_spectrum_ready = true;
        }
        return append_csv_field(row, row_cap, pos, ctx->laplacian_spectrum);
    case COL_SPANNING_TREE_COUNT:
        if (!ctx->spanning_trees_ready) {
            compute_spanning_tree_count(g, ctx->spanning_trees,
                                        sizeof(ctx->spanning_trees));
            ctx->spanning_trees_ready = true;
        }
        return append_csv_field(row, row_cap, pos, ctx->spanning_trees);
    case COL_SPECTRUM:
        if (!ensure_genus_stats(ctx)) return false;
        return append_csv_field(row, row_cap, pos, ctx->genus_stats.spectrum);
    case COL_GAMMA:
        if (!ensure_genus_stats(ctx)) return false;
        snprintf(tmp, sizeof(tmp), "%d", ctx->genus_stats.gamma);
        return append_csv_field(row, row_cap, pos, tmp);
    case COL_G_MAX:
        if (!ensure_genus_stats(ctx)) return false;
        snprintf(tmp, sizeof(tmp), "%d", ctx->genus_stats.g_max);
        return append_csv_field(row, row_cap, pos, tmp);
    case COL_EXPECTED_GENUS:
        if (!ensure_genus_stats(ctx)) return false;
        snprintf(tmp, sizeof(tmp), "%.12g", ctx->genus_stats.expected);
        return append_csv_field(row, row_cap, pos, tmp);
    case COL_VARIANCE:
        if (!ensure_genus_stats(ctx)) return false;
        snprintf(tmp, sizeof(tmp), "%.12g", ctx->genus_stats.variance);
        return append_csv_field(row, row_cap, pos, tmp);
    case COL_R:
        if (!ensure_genus_stats(ctx)) return false;
        snprintf(tmp, sizeof(tmp), "%.12g", ctx->genus_stats.r);
        return append_csv_field(row, row_cap, pos, tmp);
    case COL_R_1:
        if (!ensure_genus_stats(ctx)) return false;
        snprintf(tmp, sizeof(tmp), "%.12g", ctx->genus_stats.r1);
        return append_csv_field(row, row_cap, pos, tmp);
    case COL_R_2:
        if (!ensure_genus_stats(ctx)) return false;
        snprintf(tmp, sizeof(tmp), "%.12g", ctx->genus_stats.r2);
        return append_csv_field(row, row_cap, pos, tmp);
    case COL_TOTAL_ROTATION_SYSTEMS:
        if (!ensure_genus_stats(ctx)) return false;
        snprintf(tmp, sizeof(tmp), "%" PRIu64, ctx->genus_stats.total_rotation_systems);
        return append_csv_field(row, row_cap, pos, tmp);
    case COL_HAS_NONREAL_ROOTS:
        if (!ensure_genus_stats(ctx)) return false;
        if (!ctx->has_nonreal_roots_ready) {
            ctx->has_nonreal_roots =
                genus_polynomial_has_nonreal_roots(&ctx->genus_stats);
            ctx->has_nonreal_roots_ready = true;
        }
        return append_csv_field(row, row_cap, pos,
                                ctx->has_nonreal_roots ? "1" : "0");
    case COL_COUNT:
        break;
    }

    snprintf(ctx->err, ctx->err_len, "internal unknown CSV column");
    return false;
}
...
static char *analyze_graph_to_csv(const Graph *g, const Options *options, char *err,
    ...
    AnalysisContext ctx = {
        ...
        .err = err,
        .err_len = err_len,
        .automorphism_order = "NA",
        .adjacency_spectrum = "NA",
        .laplacian_spectrum = "NA",
        .spanning_trees = "NA",
    };
    row[0] = '\0';
    size_t pos = 0;
    ...
    for (size_t i = 0; i < options->column_count; ++i) {
        if (!append_csv_column(row, GP_ROW_CAP, &pos, &ctx,
                               (ColumnId)options->columns[i])) {
            if (err[0] == '\0') snprintf(err, err_len, "CSV row exceeded internal buffer");
            free(row);
            return NULL;
        }
    }
...
static void options_set_all_columns(Options *options) {
    options->column_count = 0;
    memset(options->column_enabled, 0, sizeof(options->column_enabled));
    for (int i = 0; i < COL_COUNT; ++i) {
        options->columns[options->column_count++] = i;
        options->column_enabled[i] = true;
    }
}

static int find_column_id(const char *name) {
    for (int i = 0; i < COL_COUNT; ++i) {
        if (strcmp(name, COLUMN_SPECS[i].name) == 0) return i;
    }
    return -1;
}

static void print_column_names(FILE *stream) {
    for (int i = 0; i < COL_COUNT; ++i) {
        fprintf(stream, "%s%s", i ? "," : "", COLUMN_SPECS[i].name);
    }
}

static void parse_columns_option(Options *options, const char *text) {
    if (strcmp(text, "all") == 0) {
        options_set_all_columns(options);
        return;
    }
    ...
    for (char *token = strtok_r(copy, ",", &saveptr); token;
        ...
        if (*token == '\0') {
            free(copy);
            die("--columns contains an empty column name");
        }

        int id = find_column_id(token);
        if (id < 0) {
            fprintf(stderr, "unknown column: %s\navailable columns: ", token);
            print_column_names(stderr);
            fprintf(stderr, "\n");
            free(copy);
            exit(2);
        }
        if (options->column_enabled[id]) {
            fprintf(stderr, "duplicate column: %s\n", token);
            free(copy);
            exit(2);
        }
        options->columns[options->column_count++] = id;
        options->column_enabled[id] = true;
    }

    free(copy);
    if (options->column_count == 0) die("--columns needs at least one column");
}
...
static void print_usage(FILE *stream) {
    fprintf(stream,
            ...
            "  --columns LIST         Comma-separated CSV columns to compute and write.\n"
            "                         Default: all. Use --columns=all for the full set.\n"
            "  --help                 Show this help.\n");
    fprintf(stream, "\nAvailable columns:\n  ");
    print_column_names(stream);
    fprintf(stream, "\n");
}
...
static Options parse_options(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (strcmp(arg, "--help") == 0 || strcmp(arg, "-h") == 0) {
            ...
            if (++i >= argc) die("--columns needs a comma-separated list or all");
            parse_columns_option(&options, argv[i]);
        } else if (strncmp(arg, "--columns=", 10) == 0) {
            parse_columns_option(&options, arg + 10);
        } else {
...
static void write_csv_header(FILE *out, const Options *options) {
    for (size_t i = 0; i < options->column_count; ++i) {
        if (i > 0) fputc(',', out);
        fputs(COLUMN_SPECS[options->columns[i]].name, out);
    }
    fputc('\n', out);
}
