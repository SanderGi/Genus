...
"""Draw vertoces and edges on a 3D torus."""
...
var("u v t")
a, b = 2, 1
x = (a + b * cos(u)) * cos(v)  # type: ignore
y = (a + b * cos(u)) * sin(v)  # type: ignore
z = b * sin(u)  # type: ignore
...
def edge_on_torus(p1, p2, wrap_around=False, go_around=False, color="blue"):
    ...
    u_interp = (1 - t) * uh1 + t * uh2  # type: ignore
    v_interp = (1 - t) * vh1 + t * vh2  # type: ignore
    if wrap_around:
        u_interp = (1 - t) * uh2 + t * uh1  # type: ignore
    if go_around:
        v_interp = (2 - 2 * t) * vh2 + 2 * t * vh1 + 1 * pi / 2  # type: ignore
    ...
    edge_curve = parametric_plot(
        [x_interp, y_interp, z_interp], (t, 0, 1), color=color, thickness=2  # type: ignore
    )
    return edge_curve
...
# Plot the torus
T = parametric_plot3d(
    [(a + b * cos(u)) * cos(v), (a + b * cos(u)) * sin(v), b * sin(u)],  # type: ignore
    (u, 0, 2 * pi),  # type: ignore
    (v, 0, 2 * pi),  # type: ignore
    opacity=0.6,
    aspect_ratio=1,
)
