...
## Usage
To run the C program for any graph, `cd PAGE` and run `S="0" DEG="3" ADJ="adjacency_lists/3-8-cage.txt" make run`. This will compile the C program and run it. The output will be in `page.out`. The format of the adjacency lists is the number of vertices and number of edges on the first line followed by the neighbors of each vertex on the following lines. See the examples in `PAGE/adjacency_lists/`. PAGE limits materialized cycle-generation data to 1024 MiB by default so graphs with billions of candidate cycles fail with a diagnostic instead of exhausting system memory; set `MAX_CYCLE_MEMORY_MB` to a positive integer to override this limit when sufficient memory is available. Use `MallocStackLogging=1 S="0" DEG="3" ADJ="adjacency_lists/3-8-cage.txt" leaks -quiet -atExit -- ./page` to check for memory leaks on macOS. Use `S=0 DEG=7 ADJ="adjacency_lists/coHerschel.txt" lldb --file ./page` and type `r` then `bt` to debug segmentation faults. If you're on Windows, you'll probably have an easier time if you use [WSL](https://learn.microsoft.com/en-us/windows/wsl/install) or [MSYS](https://www.msys2.org/).

For graphs whose candidate-cycle tables exceed that budget, run `S="0" DEG="3" ADJ="adjacency_lists/3-8-cage.txt" make run_low_mem`. `page_low_mem.c` has the same environment-variable input and `page.out` certificate format, but generates only the facial walks through the currently selected uncovered dart and discards them after each branch. Its memory use is linear in the graph and output embedding rather than in the number of graph cycles. This is especially effective for large dense/near-quadrangular graphs; the materialized implementation usually remains faster when its candidate tables fit comfortably in memory.
