...
gcc -O3 -DLONG -march=native -o multi_genus_128 multi_genus_longtype.c -lm

or

gcc -O3 -march=native -o multi_genus_64 multi_genus_longtype.c -lm
...
// without the common vertices
{
  ...
  if (ell == 0) {
    *found = 1;
    if ((write) || (genus == filter) || (genus == filter2)) code();
    return;
  }
